...
<template>
  <div>
    ...
    <div v-else class="glass" style="background:#000;padding:16px;font-family:monospace;font-size:13px;max-height:500px;overflow-y:auto;">
      <div v-if="logLines.length===0" style="color:var(--text-dim);">等待日志...</div>
      <div
        v-for="(line, i) in logLines"
        :key="i"
        style="line-height:1.6;"
        :style="{color: line.includes('NaN') ? '#f87171' : line.includes('loss') ? '#60a5fa' : line.includes('Step') ? '#4ade80' : 'var(--text-secondary)'}"
      >{{ line }}</div>
    </div>
  </div>
</template>
